...
#define END_FMT \
	"%d lps in %10u us, %5.3f us/TLOG, %7.3f Mtlogs/s", loops, delta_us, (double)delta_us / loops, (double)loops / delta_us

	if (mask & 0x1) {
		mark_us= gettimeofday_us();
		for (int ii= 0; ii < loops; ++ii) TLOGPF(TLVL_DEBUG) << "0x1 hello " << ii << " of " << loops;
	...
	if (mask & 0x2) {
		mark_us= gettimeofday_us();
		for (int ii= 0; ii < loops; ++ii) TLOGF(TLVL_DEBUG) << "0x2 hello " << ii << " of " << loops;
		delta_us= (uint32_t)(gettimeofday_us() - mark_us);
	...
	if (mask & 0x4) {
		mark_us= gettimeofday_us();
		for (int ii= 0; ii < loops; ++ii) TLOG(TLVL_DEBUG) << "0x4 hello " << ii << " of " << loops;
		delta_us= (uint32_t)(gettimeofday_us() - mark_us);
	...
	if (mask & 0x8) {
		mark_us= gettimeofday_us();
		for (int ii= 0; ii < loops; ++ii) TRACE(TLVL_DEBUG, "0x8 hello %d of %d", ii, loops);
		delta_us= (uint32_t)(gettimeofday_us() - mark_us);
...
int main(int argc, char *argv[])
{
	...
	for (int rr= 0; rr < opt_repeat; ++rr) someNamespace::subroutine(opt_loops, opt_mask);
	return (0);
}  // main
