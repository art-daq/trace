...
static int __init trace_msgmax_setup(char *str) { KSTRVAL(str, trace_msgmax, msgmax, d); }
__setup("trace_msgmax=", trace_msgmax_setup);
static int __init trace_argsmax_setup(char *str) { KSTRVAL(str, trace_argsmax, argsmax, d); }
__setup("trace_argsmax=", trace_argsmax_setup);
static int __init trace_numents_setup(char *str) { KSTRVAL(str, trace_numents, numents, d); }
__setup("trace_numents=", trace_numents_setup);
static int __init trace_namtblents_setup(char *str) { KSTRVAL(str, trace_namtblents, namtblents, d); }
__setup("trace_namtblents=", trace_namtblents_setup);
static int __init trace_buffer_numa_node_setup(char *str) { KSTRVAL(str, trace_buffer_numa_node, trace_buffer_numa_node, d); }
__setup("trace_buffer_numa_node=", trace_buffer_numa_node_setup);
static int __init trace_allow_printk_setup(char *str) { KSTRVAL(str, trace_allow_printk, trace_allow_printk, d); }
__setup("trace_allow_printk=", trace_allow_printk_setup);
static int __init trace_lvlS_setup(char *str) { KSTRVAL(str, trace_lvlS, trace_lvlS, llu); }
__setup("trace_lvlS=", trace_lvlS_setup);
static int __init trace_lvlM_setup(char *str) { KSTRVAL(str, trace_lvlM, trace_lvlM, llu); }
...
static ssize_t trace_proc_control_write(struct file *fil, const char __user *src_p, size_t siz, loff_t *off)
{
	char kernelBuffer[1024];
	char *sptr;
	int cc;
	cc= (siz < (sizeof(kernelBuffer) - 1)) ? siz : (sizeof(kernelBuffer) - 1);
	if (copy_from_user(kernelBuffer, src_p, cc) != 0) return -1;
...
static ssize_t trace_proc_control_read(struct file *fil, char __user *dst_p, size_t siz, loff_t *off)
{
	char kernelBuffer[1024];
	int cc;
	...
	if (*off >= cc) return 0;
	if (copy_to_user(dst_p, &kernelBuffer[*off], cc - *off) != 0) return -1;
	cc= cc - *off;
	*off+= cc;
	return cc;
}
...
static int trace_proc_buffer_mmap(struct file *file, struct vm_area_struct *vma)
{
	int sts;
	unsigned long pfn;
...
static ssize_t trace_proc_buffer_read(struct file *fil, char __user *dst_p, size_t siz, loff_t *off)
{
	long must_check;
	long lcl_off;
	long dst_off= 0;
	...
	int irq, struct irqaction *action)
{
	...
	TRACE(29, "cpu=%d irq=%d", raw_smp_processor_id(), irq);
	...
	int irq, struct irqaction *action, int ret)
{
	// comment out since I don't use action ptr. unsigned long flags;
	//local_irq_save(flags);
	TRACE(30, "cpu=%d irq=%d ret=%d", raw_smp_processor_id(), irq, ret);
	//local_irq_restore(flags);
}  // my_trace_irq_exit
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(2, 6, 36)
	void *new, unsigned int vec_nr)
{
#else
	struct softirq_action *x, struct softirq_action *y)
{
	...
	struct pt_regs *regs, long id)
{
	...
	if (syscall_nr == id) TRACE(25, "cpu=%d syscall=%ld", raw_smp_processor_id(), id);
	else
	...
	struct pt_regs *regs, long ret)
{
	...
	if (syscall_ret == ret) TRACE(26, "cpu=%d syscall=%d ret=0x%lx (%ld)", raw_smp_processor_id(), syscall_nr, ret, ret);
	else
		TRACE(26, "cpu=%d syscall=%d ret=0x%lx (%ld), 0x%lx (%ld)", raw_smp_processor_id(), syscall_nr, syscall_ret, syscall_ret,
			  ret, ret);
...
static void _rcu_utilization(void *ignore, struct pt_regs *regs, long ret)
{
	TRACE(32, "cpu=%d rcu_utilization", raw_smp_processor_id());
}

static void _rcu_stall_warning(void *ignore, struct pt_regs *regs, long ret)
{
	TRACE(33, "cpu=%d rcu_stall_warning", raw_smp_processor_id());
}

static void _alarmtimer_fired(void *ignore, struct pt_regs *regs, long ret)
{
	TRACE(34, "cpu=%d alarmtimer_fired", raw_smp_processor_id());
}

static void _nmi_handler(void *ignore, struct pt_regs *regs, long ret) { TRACE(35, "cpu=%d nmi_handler", raw_smp_processor_id()); }

static void _mm_lru_insertion(void *ignore, struct pt_regs *regs, long ret)
{
	TRACE(36, "cpu=%d mm_lru_insertion", raw_smp_processor_id());
}

static void _mm_lru_activate(void *ignore, struct pt_regs *regs, long ret)
{
	TRACE(37, "cpu=%d mm_lru_activate", raw_smp_processor_id());
}

static void _powernv_throttle(void *ignore, struct pt_regs *regs, long ret)
{
	TRACE(38, "cpu=%d powernv_throttle", raw_smp_processor_id());
}

static void _cpu_frequency(void *ignore, struct pt_regs *regs, long ret)
{
	TRACE(39, "cpu=%d cpu_frequency", raw_smp_processor_id());
}

static void _cpu_frequency_limits(void *ignore, struct pt_regs *regs, long ret)
{
	TRACE(40, "cpu=%d cpu_frequency_limits", raw_smp_processor_id());
}

static void _cpu_migrate_begin(void *ignore, struct pt_regs *regs, long ret)
{
	TRACE(41, "cpu=%d cpu_migrate_begin", raw_smp_processor_id());
}

static void _mm_migrate_pages(void *ignore, struct pt_regs *regs, long ret)
{
	TRACE(42, "cpu=%d mm_migrate_pages", raw_smp_processor_id());
}

static void _ipi_entry(void *ignore, struct pt_regs *regs, long ret) { TRACE(43, "cpu=%d ipi_entry", raw_smp_processor_id()); }

static void _ipi_exit(void *ignore, struct pt_regs *regs, long ret) { TRACE(44, "cpu=%d ipi_exit", raw_smp_processor_id()); }

static void _timer_expire_entry(void *ignore, struct pt_regs *regs, long ret)
{
	TRACE(45, "cpu=%d timer_expire_entry", raw_smp_processor_id());
}

static void _timer_expire_exit(void *ignore, struct pt_regs *regs, long ret)
{
	TRACE(46, "cpu=%d timer_expire_exit", raw_smp_processor_id());
}

static void _hrtimer_expire_entry(void *ignore, struct pt_regs *regs, long ret)
{
	TRACE(47, "cpu=%d hrtimer_expire_entry", raw_smp_processor_id());
}

static void _hrtimer_expire_exit(void *ignore, struct pt_regs *regs, long ret)
{
...
{
	if (strcmp(tp->name, "sched_switch") == 0) tracepoint_probe_unregister(tp, _sched_switch_hook, NULL);
	else if (strcmp(tp->name, "irq_handler_entry") == 0)
...
#else
	err= register_trace_sched_switch(_sched_switch_hook REGISTER_NULL_ARG);
...
MODULE_LICENSE(
	"GPL v2"); /* It is for anyone/everyone, I don't care as long as it works for me, and besides I don't want to taint the kernel */
#endif         // MODULE */
