...
int64_t TRACE_CNTL(const char *_name, const char *_file, int nargs, const char *cmd, ...)
{
	...
	if (strcmp(cmd, "printfd") == 0) {
		...
		for (int ii= 0; ii < ((nargs < max) ? nargs : max); ++ii) { arg_i[ii]= va_arg(ap, int); }
		retval= traceCntl(_name, _file, nargs, cmd, arg_i[0], arg_i[1], arg_i[2], arg_i[3], arg_i[4], arg_i[5], arg_i[6], arg_i[7],
						  arg_i[8], arg_i[9]);
	} else {
		int max= (int)(sizeof(arg_i) / sizeof(arg_i[0]));
		for (int ii= 0; ii < ((nargs < max) ? nargs : max); ++ii) { arg_i[ii]= va_arg(ap, int); }
