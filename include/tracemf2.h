...
static void vmftrace_user(struct timeval*, int TID, uint16_t lvl, const char* insert, const char* file, int line,
  ...
  */
  ...
  if (nargs)
    vsnprintf(&(obuf[printed]), (printed < (int)sizeof(obuf)) ? sizeof(obuf) - printed : 0, msg, ap);
  ...
  switch (lvl) {
    case TLVL_ERROR:
      ::mf::LogError(namebuf) << obuf;
      break;
    case TLVL_WARNING:
      ::mf::LogWarning(namebuf) << obuf;
      break;
    case TLVL_INFO:
      ::mf::LogInfo(namebuf) << obuf;
      break;
    case TLVL_DEBUG:
      ::mf::LogDebug{namebuf, file, line} << obuf;
      break;
    default:
      ::mf::LogDebug{namebuf, file, line} << std::to_string(lvl) << ": " << obuf;
      break;
  }
}
 ...
 public:
  ...
  inline void str() {
    ...
    if (do_s) {
      ...
      TRACE_LOG_FUNCTION(&lclTime, tid_, lvl_, _insert, file_, line_, argCount, msg, TRACE_STREAMER_EXPAND(args));
    }
  }
  ...
  inline void format(bool isFloat, bool isUnsigned, std::string length, std::ios::fmtflags flags) {
    ...
    if (isFloat) {
      ...
      if ((flags & (fixed | scientific)) == (fixed | scientific)) /*AND*/ {
        ...
        msg += flags & uppercase ? "E" : "e";
      } else {
        msg += flags & uppercase ? "G" : "g";
      }
    } else {
      msg += length;
      if (isUnsigned) {
        if (flags & hex) {
          msg += flags & uppercase ? "X" : "x";
        } else if (flags & oct) {
          msg += "o";
        } else {
          msg += "u";
  ...
  inline TraceStreamer& operator<<(
      void* const& p)  // Tricky C++...to pass pointer by reference, have to have the const AFTER the type
  {
