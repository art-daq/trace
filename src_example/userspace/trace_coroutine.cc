...
int main()
{
	...
	while (evens_tmpl.move_next()) { TLOG() << "template got: " << evens_tmpl.current_value(); }
	...
	while (evens.move_next()) { TLOG() << "concrete got: " << evens.current_value(); }
...
int main()
{
	std::cerr << "trace_coroutine: this program was built with a compiler/standard"
				 " that does not support coroutines (need C++20 or later)."
			  << std::endl;
	return 1;
}
