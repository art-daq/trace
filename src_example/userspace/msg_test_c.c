...
int main(int argc, char *argv[])
{
	...
	printf("w/ %7s=tracePrint_cntl and obuf[%d]: ", tracePrint_cntl, msgmax);
	fflush(stdout);
	if (args == 1) {
		for (ii= 0; ii < opt_loops; ++ii) {
			TRACE(TLVL_INFO, argv[optind]);
			if (opt_sleep && ii < opt_loops) sleep(opt_sleep);
		}
		return (0);
	} else
