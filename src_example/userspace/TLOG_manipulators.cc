...
int main(int argc __attribute__((__unused__)), char *argv[] __attribute__((__unused__)))
{
	for (unsigned ii= 0; ii < 2; ++ii)
		TLOG(TLVL_DEBUG) << "ii+10=" << ii + 10 << " " << std::fixed << std::setprecision(1) << 123.456;
	for (unsigned ii= 0; ii < 2; ++ii) {
		TLOG(TLVL_DEBUG) << "ii+10=" << ii + 10 << std::hex << " " << ii + 10 << " " << 789.012 << " " << std::setprecision(1)
						 << 123.456;
		TLOG(TLVL_TRACE) << "ii+10=" << ii + 10 << std::hex << " " << ii + 10 << " " << 789.012 << " " << std::setprecision(2)
						 << 123.456;
	}
	...
	const char *space= "                    ";
	TLOG(2) << '1' << " " << two << " " << thr << " " << fou << " " << fiv << " " << six << " " << sev << " " << eig << " " << nin
			<< " " << ten << " " << ele;
...
#endif
				<< (void *)0x12345 << " " << eit << " " << 99;
	TLOG() << (void *)0x12345678 << " " << eit << " sizeof(trace_ptr_t)=" << sizeof(trace_ptr_t) << " " << (long double)3.0 << " "
		   << 3 << " " << reinterpret_cast<const void *>(0x1234) << " " << reinterpret_cast<const void *>(0x5678) << " "
