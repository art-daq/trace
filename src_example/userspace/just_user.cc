  ...
  while (1) {
    ...
    static struct option long_options[] = {
        ...
        {"verbose", optional_argument, 0,
         'v'},  // --verbose can take optional arg (not so for just -v, so -vvv is allowed)
        {0, 0, 0, 0}};
    opt = getopt_long(argc, argv,
                      "?hvVl:t:",  // optional args use "::".  NOTE: -v has no arg but --verbose can have optional
                      long_options, NULL);
    if (opt == -1) break;
    switch (opt) {
      case '?':
      ...
      case 'v':
        if (optarg)
          opt_v = strtoul(optarg, 0, 0);
        else
          ++opt_v;
        break;  // optional is for when coming from long option
      case 'l':
...
int main(int argc, char *argv[]) {
  ...
  if (strcmp(opt_test, "") == 0) {
    ...
    // FILE *fp(popen("head -379 $HOME/.bashrc", "r")); // crash  379 lines in 16742 -- line 379 has HISTTIMEFORMAT="%a
    // %m/%d %H:%M:%S  "  FILE *fp(popen("cat $HOME/t.t", "r")); // 63125 KB of lines of 100 chars 123456... - works
    std::string result;
    if (fp) {
      char buffer[128];
      while (!feof(fp)) {
        if (fgets(buffer, 128, fp) != NULL) result += buffer;
      }
    ...
    // TRACE( 1, ostr <<"I would like to see what is it, not it's address", 1 ); // THIS prints to the screen and prints
    // an address -- not useful  TRACE_( 1, (ostr<<"xx").str(), 2 ); these shenanigans don't compile
    ...
    while (opt_loops--) {
      TRACE(0, "this is an int: " + std::to_string(55LL));  // this does same/similar thing as "<<" into stream (but
                                                            // without the stream constructor overhead
    }
  }
#endif
  else {
