...
int count_bits( unsigned input ) {
	int bits=0;
	for (unsigned bb=0; bb<(sizeof(input)*8 /*bitsPerByte*/); ++bb) if (input&(1<<bb)) ++bits;
	return bits;
}
...
int main(int argc, char *argv[])
{
	...
	unsigned tests_mask= 0x3ff; /* all tests */
	...
	while ((opt= getopt(argc, argv, "?hl:n")) != -1) {
		...
		case 'h':
			...
			break;
		case 'l':
			opt_loops= (int)strtoul(optarg, NULL, 0);
			break;
		case 'n':
			opt_normalize=true;
		}
...
#define CONTINUE                      \
	...
	for (int jj= 0; jj < 4; ++jj) {
		...
		if (1 & tests_mask) {
			...
			fprintf(stderr,"%c %2u/%u\r",prop[test&0x3],test+1,total_tests); fflush(stderr);
...
#ifndef __OPTIMIZE__
			//STRT_PRN(" 0x200 - 2 args%s%s ", " - NoTLOG - ", "NOT Optimized.");
#else
			//STRT_PRN(" 0x200 - 2 args%s%s ", " - NoTLOG - ", "OPTIMIZED out.");
#endif  // __OPTIMIZE__
			TRACE_CNTL("reset");
			unsigned total=0;
			mark= gettimeofday_ns();
