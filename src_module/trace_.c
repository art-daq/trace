/*  This file (trace.c) was created by Ron Rechenmacher <ron@fnal.gov> on
    ...
    $RCSfile: trace_.c,v $
    rev="$Revision: 1025 $$Date: 2018-11-02 16:43:50 -0500 (Fri, 02 Nov 2018) $";
    */
...
static int __init trace_lvlS_setup(char *str) { KSTRVAL(str, trace_lvlS, trace_lvlS, lu); }
__setup("trace_lvlS=", trace_lvlS_setup);
static int __init trace_lvlM_setup(char *str) { KSTRVAL(str, trace_lvlM, trace_lvlM, lu); }
...
static void my_trace_hirq_enter(
  ...
  TRACE(29, "hirqenter: cpu=%d irq=%d", raw_smp_processor_id(), irq);
...
static void my_trace_hirq_exit(
  ...
  // comment out since I don't use action ptr. unsigned long flags;
  // local_irq_save(flags);
  TRACE(30, "hirqexit: cpu=%d irq=%d ret=%d", raw_smp_processor_id(), irq, ret);
  // local_irq_restore(flags);
}  // my_trace_irq_exit
...
static void my_trace_sys_exit(
  ...
  if (syscall_ret == ret)
    TRACE(26, "sys_exit: cpu=%d syscall=%d ret=0x%lx (%ld)", raw_smp_processor_id(), syscall_nr, ret, ret);
  else
    TRACE(26, "sys_exit: cpu=%d syscall=%d ret=0x%lx (%ld), 0x%lx (%ld)", raw_smp_processor_id(), syscall_nr,
...
#else
  err = register_trace_sched_switch(my_trace_sched_switch_hook REGISTER_NULL_ARG);
...
static void trace_sched_switch_hook_remove(void) {
  ...
  unregister_trace_sched_switch(my_trace_sched_switch_hook REGISTER_NULL_ARG);
#endif
...
MODULE_LICENSE("GPL"); /* It is for anyone/everyone, I don't care as long as it works for me, and besides I don't want
                          to taint the kernel */
#endif                 // MODULE */
